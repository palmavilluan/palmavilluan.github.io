...
async function downloadFromFolder() {
  ...
  return {images, numberOfElements};
}
...
document.querySelector('a-scene').addEventListener('loaded', async function () {
  ...
  var bigdata = await downloadFromFolder();
  var numberOfElements = bigdata.numberOfElements;
  var anzahlBilderProWand = Math.ceil(numberOfElements / 4);

  let data = bigdata.images;
  ...
  cameraButton.addEventListener('click', () => {
      console.log("cameraEntity clicked");
